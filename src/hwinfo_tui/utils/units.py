...
from ..data.sensors import Sensor, SensorGroup, SensorInfo
...
class UnitFilter:
    ...
    def __init__(self, max_units: int = 2) -> None:
        ...
        self.allowed_units: Set[Optional[str]] = set()
        self.excluded_sensors: List[Tuple[str, Optional[str], str]] = []
    
    @staticmethod
    def extract_unit(sensor_name: str) -> Optional[str]:
    ...
    def _create_exclusion_reason(self, excluded_unit: Optional[str]) -> str:
    ...
    def create_sensor_groups(self, sensors: Dict[str, Sensor]) -> List[SensorGroup]:
        ...
        unit_groups: Dict[Optional[str], List[Sensor]] = {}
    ...
    def get_allowed_units(self) -> Set[Optional[str]]:
        """Get the set of allowed units."""
        return self.allowed_units.copy()
    
    def get_excluded_sensors(self) -> List[Tuple[str, Optional[str], str]]:
        """Get list of excluded sensors with their reasons."""
        return self.excluded_sensors.copy()


def validate_sensor_compatibility(sensor_names: List[str], max_units: int = 2) -> Tuple[List[str], List[str], Set[Optional[str]]]:
    """
...
def get_unit_display_info(units: Set[Optional[str]]) -> Dict[str, str]:
